...
Now we can plot the temperature and salinity time series for each depth. We will use `facet_wrap()` to create separate plots for each depth.
...
We can also calculate the monthly climatology for temperature and salinity. We will use `lubridate` to extract the month from the `TIME` variable and then calculate the mean for each month.
